...
async function currentRows(store) {
  const [{ c }] = await store.query(`SELECT count() AS c FROM bench__priorities`);
  return Number(c);
}

export function initBench({ store }) {
  let created = false;
  const scaleButtons = [...el("act-scale").querySelectorAll("button[data-n]")];
  for (const b of scaleButtons) {
    b.disabled = false;
    b.onclick = async () => {
      const n = Number(b.dataset.n);
      for (const sb of scaleButtons) sb.disabled = true;
      ...
      try {
        ...
        const rows = await currentRows(store);
        el("genmsg").textContent =
          `${rows.toLocaleString()} rows live in this tab (${fmtMs(performance.now() - t0)} to generate)`;
      } finally {
        for (const sb of scaleButtons) sb.disabled = false;
